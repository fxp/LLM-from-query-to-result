...
| 01 | [L3：从莎士比亚训出一个 GPT](01-L0-training.md) | `00_train/` | 预训练循环、loss 为什么从 10.8 降到 4.5 |
| 02 | [L4：24 秒把"接龙莎翁"变成"答 Paris"](02-L0.5-sft.md) | `00b_sft/` | SFT、loss masking、为什么需要 124M base 才能泛化 |
| 03 | [L7：浏览器里那一个个蹦出来的字](03-L1-app.md) | `01_app/` | SSE、async generator、~80 行前端 |
| 04 | [L8：Chat 客户端的最小本质](04-L2-chat-client.md) | `02_agent/` | build prompt → POST → relay，删掉所有不必要的抽象 |
| 05 | [L6：自己写 KV cache 的推理服务](05-L3-inference-server.md) | `03_model/` | 不靠 transformers runtime、prefill vs decode、HF mirror 自动 fallback |
...
| 08 | [L1：一次矩阵乘在 GPU 上到底怎么跑](08-L5-gpu.md) | `05_gpu/` | naive vs tiled vs cuBLAS、Triton flash-attn |
| 09 | [端到端 trace：从一句 query 到一次浮点乘法](09-end-to-end-trace.md) | — | 把 9 层串起来，一个 token 的完整旅程 |
| 10 | [L5：教一个 124M 模型用工具](10-L0.6-agent.md) | `00c_agent_sft/` + `agent_loop.py` | ReAct SFT，loss masking 关键细节，agent 不靠更聪明靠工具扩展 |

## 适合谁读

- **想搞清楚 ChatGPT 怎么"运转"的工程师**：这本书把抽象塌缩成具体——每一层 < 300 行代码，可改可调试。
- **想入门 LLM 训练的人**：L3 + L4 是完整的 pretrain + SFT 流程，CPU 几分钟、GPU 几秒能跑完。
- **想理解推理性能的人**：L6 + L1 把 KV cache、prefill/decode 拆成 prefill/decode、Tensor Core、flash-attention 都讲了。
- **想写自己的 LLM 服务的人**：L7/L8/L6 给了一个最小的、能跑的 chat 服务（前端 + SSE + 推理引擎），不到 300 行。
