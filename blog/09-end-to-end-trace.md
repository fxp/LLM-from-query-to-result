...
> [← L1 GPU](08-L5-gpu.md) ｜ 配套源码：[`examples/trace.md`](https://github.com/fxp/LLM-from-query-to-result/blob/main/examples/trace.md)

到这里我们走完了 9 篇。每一层独立讲过——L3 训出 model、L4 SFT 让它能答问、L1-L1 把它服务出来。这一篇把所有层串起来：跟着用户在浏览器输入的那一句 query "What is the capital of France?"，看它穿过 9 层、最终在屏幕上变成 "Paris." 的全过程。
...
## t=12 ms · L2：tensor 流过 12 层 transformer
...
input_ids [1, 12]
   ...
   │          → scaled_dot_product_attention (内部调 cuBLAS + softmax + cuBLAS, 在 L1 里)
...
## t=12.06 ms（一个 matmul 之内）· L1：tensor 变成机器指令
...
> 如果用我们手写的 `matmul_tiled.cu`：每个 C 元素 HBM 读 2K = 1536 个 float = 6 KB，加上数据复用大概 5× 减少。但没用 Tensor Core，跑 fp32，~9 TFLOPS。**比 cuBLAS 慢 7×**——这就是 [L1](08-L5-gpu.md) 那篇展示的差距。

**这一层的关键**：到 L1 我们已经忘了 "France" 这个字了。剩下的全是浮点数乘加。

---

## t=14 ms · 回到 L6：第一个 token 出来
...
## t=14.5 ms · L8 → L7 → 浏览器：第一个 token 显示

```
L6 SSE: data: {"token": " Paris"}\n\n
   │
   ▼ L8 (urllib 行迭代器)
yield {"type": "token", "v": " Paris"}
   │
   ▼ L7 (FastAPI generator → StreamingResponse)
...
## t=17 ms · L6：第二个 token，用 KV cache
...
argmax 出来：id 13 = `'.'`。yield 给 L8。屏幕上看到 "Paris."。

---

## t=20 ms · L6：第三个 token，EOT
...
L6 stream 结束。L8 yield `{"type": "done"}`。L7 yield `data: {"done": true}\n\n`。浏览器收到 `done` event，render `✓ done`。
...
| **L2 prefill (124M, T=12)** | **1.79 ms** | **13%** |
| 生成第 1 个 token (sample + KV cache 回传) | ~2 ms | 14% |
| **L2 decode 第 2 个 token** | **2.63 ms** | **19%** |
| **L2 decode 第 3 个 token (EOT)** | **2.63 ms** | **19%** |
| 总计端到端 | **~14 ms** | 100% |
...
| 层 | "形态" | 量级 |
|---|---|---|
| L7 | HTTP body | 80 字节 in，几 KB out (SSE 帧) |
| L8 | HTTP request | 1 个 (POST /generate) |
| L6 | tokens | 12 prompt + 3 generated |
| L6 | forward 次数 | 1 prefill + 2 decode = 3 |
| L2 | matmul 次数 | (12 layer × 6 + 1 lm_head) × 3 = 219 个 matmul |
| L1 | 浮点运算 | ~5 GFLOP prefill + 2 × 5 GFLOP decode ≈ 15 GFLOP |
| L1 | HBM 流量（权重） | 单次 forward ~500 MB，总计 ~1.5 GB |
| 时间 | end-to-end | 14 ms |
