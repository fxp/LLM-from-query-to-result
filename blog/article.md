...
│ L2   04_transformer/  GPT-2 架构（330 行）+ BPE（230 行）    │
│  训练                                                        │
│ L3   00_train/        预训练循环（莎士比亚 → 7M GPT, 12 秒） │
│ L4   00b_sft/         指令 SFT（instruction tuning, 28 秒）  │
│ L5   00c_agent_sft/   Agent SFT（ReAct + tools, 33 秒）      │
│  推理与应用                                                  │
│ L6   03_model/        推理服务（手写 KV cache）              │
│ L7   01_app/          App / Web UI + SSE 流式输出            │
│ L8   02_agent/        Chat / Agent 循环（ReAct loop）        │
└──────────────────────────────────────────────────────────────┘
```
...
## L3 → L4 → L5：从随机权重到 instruct agent，70 秒
...
所以 L4 还有 path B：用 OpenAI 公开的 gpt2-124M 当 base，同样 SFT 我的 242 条 Q/A。**5090 上 35 秒**。然后：
...
L4 让 model 能答问，但仍是"凭脑补答"。问 "1234 + 5678" 它答错。我们要让它**调用工具**。
...
def chat(req: ChatRequest):
    def sse():
        for event in run_agent(req.query):  # this is a generator from L8
            yield f"data: {json.dumps(event)}\n\n"
    return StreamingResponse(sse(), media_type="text/event-stream")
```

整个 L7 = 80 行 HTML/JS + 50 行 Python。详见 [blog/03-L1-app](03-L1-app.md)。

### 推理服务自己写 KV cache

L6 接收 prompt，跑 forward，流式吐 token。最有意思的是 KV cache。
...
```
t=0     用户按 Enter
t=1ms   L7 FastAPI 收到 POST /chat
t=2ms   L7 调 run_agent(query) → L8 build prompt → POST /generate to L6
t=5ms   L6 BPE encode prompt → 12 tokens
t=6ms   model.step(input_ids) prefill (1.79 ms GPU)
t=8ms   logits[0,-1] argmax → "THOUGHT:" token → L6 yield → L7 SSE → 浏览器看到 💭
...
错一个 token，行为就坏。我在 L5 的第一版整段 OBSERVATION mask，model 训完循环 emit ACTION——花了 30 分钟才定位。修复后 1 行代码改动，立刻 work。

### 2. 现代 GPU 太快反而暴露架构 bottleneck

5090 上手写 tiled matmul 比 naive 只快 1.3×，A100 上是 6×。原因：5090 的 HBM3e 带宽 3 TB/s，naive 实现已经不再 memory-bound——L8 cache 吃住了大部分复读。
...
完整 cold-start (5090, CN 区):
  ...
  L3 train               12 秒
  L4 path A SFT        26 秒
  L4 path B SFT (含下载) ≈ 6 min 30 秒  ← 一次性 124M 权重下载
  L5 agent SFT         26 秒
  L1 build + benchmark   10 秒
  E2E web                 5 秒
─────────────────────────────────
总计 (含下载)              ≈ 17 分钟
总计 (无下载)              ≈ 100 秒
...
跳过 path B（不需要 OpenAI 权重），整个 from-scratch 路径——clone + install + L3 + L4A + L6 启动 + 浏览器答 "Paris."——**60 秒以内**。
