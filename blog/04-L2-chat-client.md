...
> [← L1 web app](03-L1-app.md) ｜ 代码：[`02_agent/`](https://github.com/fxp/LLM-from-query-to-result/tree/main/02_agent) ｜ [下一篇 →](05-L3-inference-server.md)

L2 在我心里是这本书最有故事的一层。**它一开始有 200 行 + Anthropic SDK 依赖，最后变成 ~80 行 + 标准库**——不是因为我写得更好了，而是因为我意识到这一层不应该做 agent。
...
为什么？因为这个 repo 的整条招牌是 **"LLM from scratch"**——L4 自己实现 GPT-2 small (124M, 2019)，然后 L3 host 它给 L2 用。但 GPT-2 做不了 tool-use agent：
...
我面对一个选择：
- **(a)** 把 L4 升级到能 tool-use 的 instruct 模型（Qwen-2.5-Coder 等）。意味着重写 BPE 适配新 tokenizer，重写 KV cache 适配新模型变体，改训练循环。一个月的活儿。
...
2. **POST → L3**：发 HTTP 请求触发推理
3. **Relay SSE**：把 L3 的 token 流翻译成 L1 要的 event 流
...
def run_agent(query: str) -> Iterator[dict]:
    ...
    try:
        ...
        yield {"type": "error", "message": f"can't reach L3 at {L3_URL}: {exc.reason}"}
```
...
`Q: ...\nA:` 这种格式直接对齐 [L0.5](02-L0.5-sft.md) SFT 数据：
...
L1 调 L2 是 Python 函数调用（同进程）。但 L2 调 L3 是 **HTTP**：

```python
urllib.request.urlopen("http://localhost:9000/generate", data=payload)
```

为什么？因为 **L3 是个独立服务**——它有自己的生命周期（model 加载几秒）、自己的资源（GPU memory），可能跟 L1 跑在不同机器上。

这个边界在生产里很重要：
- L1 可能 horizontal scale（多个 web server 实例）
- L3 是垂直 scale（一个有 GPU 的大机器）
- L2 + L1 同进程：开销小，简单
- L2 → L3：HTTP 跨进程/跨机器，付一次网络往返开销，换来横向解耦

我们 demo 里 L1 + L2 都在 `01_app/backend/main.py` 这个 uvicorn 进程，L3 是 `03_model/server.py` 另一个 uvicorn 进程。两个进程独立启动、独立崩溃。

## 关键 3：流式 relay

L3 SSE 的格式是 `data: {"token": " Paris"}\n\n`（每一个 token 一帧）。L1 要的格式是 `{"type": "token", "v": " Paris"}`（每个 event 一个 dict）。L2 干的就是这个翻译。
...
这意味着 L3 yield 一个 token，L2 立刻 yield 一个 event，L1 立刻 yield 一个 SSE 帧，浏览器立刻 render。**端到端没有任何 buffer**。
...
这个原则也体现在其他层：
- L3 把 transformers runtime 删掉了 → 留下 KV cache 自己写
- L4 把 tiktoken 删掉了 → 留下 BPE 自己写
- L0 把 OpenAI 权重删掉了（默认）→ 留下训练循环本身
...
```
[L2] prompt: 'Q: What is the capital of France?\nA:'
...
每个 token 来自 [L3](05-L3-inference-server.md)，L3 调 [L4 GPT.step()](06-L4-transformer.md)，每个 token 都是我们自己训的 weights 算出来的。

## 下一篇

L2 把 prompt POST 给 L3。L3 是个 FastAPI 服务，加载 ckpt，跑 forward，流式吐 token——不依赖 transformers runtime。我们自己写了 KV cache，下一篇细讲。

[L3 — 自己写 KV cache 的推理服务 →](05-L3-inference-server.md)
