...
BASE_CKPT = HERE.parent / "00_train" / "out" / "ckpt.pt"
OUT_CKPT = HERE / "out" / "sft.pt"
DATA = HERE / "data.json"

EOT_ID = 50256  # GPT-2's <|endoftext|>
IGNORE = -1     # tells F.cross_entropy(ignore_index=-1) to skip these positions

# SFT hyperparameters: small LR (we don't want to nuke pretrained features)
# and a moderate number of epochs over a tiny dataset.
LR = 1e-4
EPOCHS = 50
BATCH_SIZE = 8
GRAD_CLIP = 1.0
...
def main() -> None:
    ...
    sample_qs = ["What is the capital of France?",
                 "What is 2 plus 2?",
    ...
    print(f"\nSFT for {EPOCHS} epochs, batch_size={BATCH_SIZE}, lr={LR}\n")
    t0 = time.time()
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 5 == 0 or epoch == EPOCHS - 1:
            print(f"epoch {epoch:>3} | avg loss {epoch_loss/n_batches:6.3f} "
