# L1 · GPU 基础层

**一句话**：L2 里的每一个 `nn.Linear` 和 `attention` 本质都是矩阵乘。这一层告诉你一次矩阵乘在 GPU 上到底怎么跑，以及为什么写对了能快 10×。

源码：[`05_gpu/`](https://github.com/fxp/LLM-from-query-to-result/tree/main/05_gpu)

## 为什么矩阵乘是核心

跑一次 GPT-2 forward，我们做过 profiling：

| 操作 | 占比 |
|---|---|
| matmul（Linear / Q·K / attn·V） | ~92% |
| softmax, layernorm, gelu     | ~5% |
| embedding lookup, reshape     | ~3% |

**所以 LLM 推理/训练性能 ≈ matmul 性能**。想理解 GPU 上的 LLM，就得理解 GPU 上的 matmul。

## GPU 的内存层级（这是理解一切的起点）

```
     ┌─────────────────────────────────────────────┐
     │  HBM / Global Memory    40-80 GB, ~1.5 TB/s │  ← 模型权重放这
     └──────────────────┬──────────────────────────┘
                        │ load / store (慢)
     ┌──────────────────▼──────────────────────────┐
     │  L8 Cache                ~40 MB, ~4 TB/s    │
     └──────────────────┬──────────────────────────┘
                        │
     ┌──────────────────▼──────────────────────────┐
     │  Shared Memory (per SM)  ~100 KB, ~20 TB/s  │  ← 分块算时把数据搬这里
     └──────────────────┬──────────────────────────┘
                        │
     ┌──────────────────▼──────────────────────────┐
     │  Registers (per thread)  ~256×32-bit        │  ← 计算在这里发生
     └─────────────────────────────────────────────┘
```

GPU 算得比读得快太多（算力 ~300 TFLOPS vs HBM ~1.5 TB/s）。**"朴素 matmul"慢不是因为算不过来，是因为一直在读 HBM**。优化的核心就一件事：**把数据搬到 shared memory 分块复用**。

## 两个 demo

### 1. 手写 CUDA matmul：朴素 vs 分块

```
  matmul_naive.cu   朴素版：每个线程算 C[i,j] 的一个元素，读 2K 次 HBM。
  matmul_tiled.cu   分块版：一个 block 协作算 C 的一个 tile，读 2K/TILE 次。
```

在 A100 上大约 `naive ≈ 2 TFLOPS`，`tiled ≈ 10 TFLOPS`，`cuBLAS ≈ 250 TFLOPS`。
我们的 tiled 版离 cuBLAS 还差 20×——那部分差距来自 Tensor Core、double buffering、vectorized loads 等更深的优化，不在本 repo 范围。但从 naive 到 tiled 这一跳（5×）能让你抓住"内存层级"这个核心思想。

### 2. Triton 版 flash-attention 雏形

Triton 是 OpenAI 做的 Python DSL，语法像 numpy 但 compile 成 CUDA。**业界真实的推理/训练 kernel 很多就是 Triton 写的**（包括 FlashAttention 的参考实现）。

`attention_triton.py` 是一个最小的 fused attention：一次 kernel 同时做 `Q·K.T`、`softmax`、`·V`，整条 attention 只读一次 HBM。和朴素 pytorch 三段式相比省掉了中间 `[B, h, T, T]` attention matrix 的 HBM 读写——当 T=4096、h=32 时这个中间矩阵有 1GB，省它就是命。

## 目录

```
05_gpu/
├── matmul_naive.cu       # 朴素 matmul，~40 行
├── matmul_tiled.cu       # 分块 matmul with shared memory，~70 行
├── attention_triton.py   # Triton fused attention，~130 行
├── benchmark.py          # 对比上面三种 + cuBLAS
└── README.md
```

- [`matmul_naive.cu`](https://github.com/fxp/LLM-from-query-to-result/blob/main/05_gpu/matmul_naive.cu)
- [`matmul_tiled.cu`](https://github.com/fxp/LLM-from-query-to-result/blob/main/05_gpu/matmul_tiled.cu)
- [`attention_triton.py`](https://github.com/fxp/LLM-from-query-to-result/blob/main/05_gpu/attention_triton.py)
- [`benchmark.py`](https://github.com/fxp/LLM-from-query-to-result/blob/main/05_gpu/benchmark.py)

## 怎么跑

需要一块 CUDA GPU。

```bash
# 1) 找出你 GPU 的 compute capability：
nvidia-smi --query-gpu=compute_cap --format=csv,noheader
# 例如 8.0 (A100) / 8.6 (3090/A40) / 8.9 (4090/4080S Ada) / 12.0 (5090 Blackwell)
#
# 2) 用 sm_<major><minor> 编译（不带点）：
cd 05_gpu
ARCH=sm_89                                             # 改成你 GPU 对应的
nvcc -O3 -arch=$ARCH matmul_naive.cu -o matmul_naive
nvcc -O3 -arch=$ARCH matmul_tiled.cu -o matmul_tiled
./matmul_naive   # 自测 + 打印 GFLOPS
./matmul_tiled

# 3) Triton + benchmark
pip install triton
python benchmark.py
```

> ⚠️ **架构必须匹配**：用错的 `-arch` 编出来的二进制驱动会"安静地不跑"——
> 给 sm_120 的 binary 在 sm_89 上跑会输出 `0.00 ms, 9000 TFLOPS` 这种胡话
> （kernel launch 失败但没抛错）。如果看到不可能的 TFLOPS 数字，先查 arch。

### 实测样例

**RTX 5090 (Blackwell sm_120, 32 GB HBM)**：

```
matmul 2048×2048×2048, fp32:
  naive CUDA    :   2.39 ms,  7.19 TFLOPS   1.0×
  tiled CUDA    :   1.86 ms,  9.24 TFLOPS   1.3×
  cuBLAS torch  :   0.25 ms, 68.94 TFLOPS   9.6×   (TF32 + Tensor Core)
attention B=4 H=16 T=1024 D=64, fp32:
  pytorch 三段   :   1.02 ms                1.0×
  triton fused  :   0.12 ms                8.5×   (省掉中间 attn matrix 的 HBM 读写)
```

**RTX 4080 SUPER (Ada Lovelace sm_89, 16 GB GDDR6X)**：

```
matmul 2048×2048×2048, fp32:
  naive CUDA    :   5.35 ms,  3.21 TFLOPS   1.0×
  tiled CUDA    :   4.30 ms,  3.99 TFLOPS   1.24×
  cuBLAS torch  :   0.51 ms, 34.0  TFLOPS   8.5×   (TF32)
attention B=4 H=16 T=1024 D=64, fp32:
  pytorch 三段   :   2.16 ms                1.0×
  triton fused  :   0.21 ms               10.3×
```

参考：A100 上 cuBLAS 大约 240 TFLOPS、tiled 11 TFLOPS、naive 1.8 TFLOPS。
不同 GPU 间的相对差异主要看 (a) HBM 带宽 (b) Tensor Core 代际。

关键观察：
- `tiled / naive` 在 A100 上 ~6×，在 5090/4080S 上只有 1.2-1.3×——
  现代 GPU 的 HBM/GDDR6X 太快，naive 已经不太被 memory-bound 卡住。
- `cuBLAS / tiled` 永远是 7-25× 因为 Tensor Core 吃 fp16/bf16/tf32 而不是 fp32。
- Triton flash-attention 在所有架构上都能 8-10× 加速 unfused — 那个是
  HBM round-trip 的纯结构性收益，跟硬件代际无关。

## 和其他层的接口

- **往上（L2）**：L2 里的 `x @ w` 最终解析成 `cuBLAS` 或 `cutlass` 的 matmul kernel。换成我们手写的 tiled 版，结果一致，只是慢 20×。换成 Triton flash-attention，结果一致，但 HBM 读写少 10×。
- **往下（硬件）**：一个 CUDA kernel 启动后，每个 SM (Streaming Multiprocessor) 拿一组 block，每个 block 有 ~1024 个线程，每 32 个线程是一个 warp，一个周期一条指令喂给整 warp——这是 GPU"算力"的来源。

## 这一层的"最小"在哪里

- **只覆盖 matmul 和 attention**：softmax / layernorm / gelu 的 kernel 思路类似（都是 "load 一 tile、在 shared 里算、store 回去"），写出来重复。
- **不追 SOTA**：手写版和 cuBLAS 差 20× 不是我们的失败——想追近 cuBLAS 需要 Tensor Core、async copy、software pipelining，每一项都是一整篇论文。这里的目标是**让你看到为什么分块是最重要的一跳**。
- **没讲多卡通信**：NCCL、all-reduce、ring topology——那是"很多个 L1 拼起来"的话题，属于下一个 repo。

---

[← L2 · Transformer 架构](04-transformer.md) | 串起来看 → [端到端 Trace](trace.md)
