...
**一句话**：拿 L3 训出的 base model（学会"续写英文"），在 ~242 条手写 Q/A 上微调几分钟，让它学会"看到问题就回答"的 instruction 格式。

源码：[`00b_sft/`](https://github.com/fxp/LLM-from-query-to-result/tree/main/00b_sft)

## 这一层为什么存在

L3 的 base model 学会的是**续写 next token**。给它 "What is the capital of France?" 它会接着写莎士比亚式的台词——因为它见过的所有训练 token 都来自莎翁的剧本。它不知道"问"和"答"是两件事。
...
├── train.py               # SFT base = L3 自训 ckpt（默认） — ~140 行
├── train_from_gpt2.py     # SFT base = OpenAI gpt2-124M  — 同套循环
├── out/sft.pt             # 训练产物（L3 base 路径，~30 MB）
...
### A. SFT on top of L3 base（完全 from-scratch，CPU 友好）
...
**RTX 5090 实测**（242 条 × 30 epochs = 930 updates，**33.9 秒**）：
...
A: ...                                       ← 知道事实，但写文章风、不停
  Q: What is 2 plus 2?
  A: 2 plus 2 is a number that is used to represent
     the number of times a given number is equal to or greater than 2.
                                              ← 不识别"问"，瞎编定义
  Q: Who wrote Hamlet?
  A: I think it was a very good book. I think it was
     written by a very good writer. I think it was written by ...
                                              ← 退化到自我重复

SFT for 30 epochs × 31 steps = 930 updates
  base=gpt2 (124M)  batch_size=8  lr=5e-5  warmup=30
epoch  0/30 | loss 1.589 |  1.2s
epoch 10/30 | loss 0.014 | 12.9s    ← 124M base 一上来 loss 就只有 1.6（vs L3 base 9.7），
epoch 20/30 | loss 0.000 | 23.5s       因为 base 早学过英文，只需要学 Q/A 格式
epoch 29/30 | loss 0.000 | 33.9s

--- AFTER SFT ---
  Q: What is the capital of France?       A: Paris.                ← in-data
  Q: What is the capital of Japan?        A: Tokyo.                ← in-data
  Q: What is 2 plus 2?                    A: 4.                    ← in-data
  ...
  Q: Who wrote Pride and Prejudice?       A: William Shakespeare.  ✗ 124M 不知道
                                                                     Jane Austen
```

**这就是为什么真正的 chat 模型从大 base 开始**：
- SFT 教的是**怎么答**（短答 + 停止），不是**答什么**（事实）。
- 124M base 在 8 GB WebText 上预训过，**已知**Iceland → Reykjavík 这类常识——SFT 只是把它"接"出来到指令格式。
- 124M 没真正"读过"Pride and Prejudice 的作者（语料里命中度低），所以 SFT 后对它仍然乱答（最常见的"author"是 SFT data 里出现最多的 William Shakespeare，default 答这个）。

更大的 base（gpt2-medium 355M、gpt2-xl 1.5B）能记住更多冷门 fact。换 base 一行命令：`HF_BASE=gpt2-xl python train_from_gpt2.py`。

### B 路径 推理速度（RTX 5090）

```
124M model on RTX 5090, batch=1, fp32:
  prefill (T=12)   : 1.79 ms/call
  decode           : 2.63 ms/token = 380 tok/s
```

CPU (M1) 同等条件下 ~10 ms/decode = 100 tok/s。GPU ~3.8× 加速。<br>
（5090 在 batch=1 下没用上多少 Tensor Core 算力——真正的吞吐优势在 batched 推理 + larger model。）
...
| `BASE_CKPT` | `00_train/out/ckpt.pt` | — | L3 base 路径 |
...
L6 会自动适配 model 大小（n_layer/n_head/n_embd 从 ckpt config 读取）。然后：
...
L8 的 prompt template (`Q: <query>\nA:`) 与本目录的 SFT 格式对齐。

## 这一层的"最小"在哪里

- **数据手写**：242 条 `data.json`，没用任何 dataset 库。每条 Q/A 多个 paraphrase 做 question-form 泛化（`What is X?` / `What's X?` / `X?`）。
- **没有 RLHF / DPO**：只 SFT 一步。Anthropic / OpenAI 的真正 instruct 模型还会用偏好数据再训一轮，但那是 L5 的活儿。
