...
**一句话**：从 0.61 MB 的莎士比亚语料出发，在 CPU 上跑 6 分钟，得到一个 7.24M 参数的 GPT，能续写出 vaguely Shakespeare-y 的英文，并能被 L6 直接服务。

源码：[`00_train/`](https://github.com/fxp/LLM-from-query-to-result/tree/main/00_train)

## 这一层为什么存在

之前 L2 实现了 GPT-2 的**架构**，但权重是 `from_pretrained("gpt2")` 从 HuggingFace 下载的——也就是说：**架构是我们自己的，但 model 的"知识"是 OpenAI 2019 年训出来的**。这和招牌"LLM from scratch"差一截。

L3 把缺失的那一段补完整：
- **数据**（`prepare.py`）：下载 Tiny Shakespeare（1.1 MB 纯文本），用 GPT-2 BPE 切成 token，存成 `train.bin / val.bin`。和 L6 用同一个 tokenizer，所以训完直接能塞回去服务。
- **训练**（`train.py`）：用**完全相同**的 L2 `GPT` 类（`04_transformer/model.py`），只是缩小到 4 层 4 头 128 维。AdamW + cosine LR + grad clip，~150 行。loss 从 10.82 降到 ~3.5。
- **采样**（`sample.py`）：加载 checkpoint，本地生成文本验证。

整条链路 + L3 闭环：**数据 → tokenize → 自己的架构 forward → loss → backward → optimizer → checkpoint → L6 加载 → L7/L8 流式调用**。
...
├── train.py          # 训练循环，~140 行（用 L2 的 GPT 类）
...
第一行的 `loss 10.815` 就是 `ln(50257) ≈ 10.82`——**完全随机的 50257-way classifier 的理论 loss**。如果你看到的初值显著高于 11，说明权重初始化有问题（早期 L2 没做 init，初值是 81）。
...
| | 训练 token | 参数量 | 知识来源 |
|---|---|---|---|
| 我们的 L3 model | 0.3M | 7M | 莎士比亚 33 个剧本 |
...
这是**故意**的：L3 演示的是**循环本身**——data → forward → loss → backward → step。把循环跑通了，剩下的全是 scale：更多数据、更大模型、更多 GPU 时。
...
5. **指令微调（SFT）**：base model 学了"续写"，但用户想要的是"问答"。在 Alpaca / OpenAssistant 数据上 SFT 几小时，base model 就会"听话"。这是 L4 的活儿，本 repo 暂未提供。

## 和其他层的接口

- **往下没有了**：L3 是源头。
- **往上（L2）**：完全复用 `04_transformer/model.py` 的 `GPT` 类。我们改的只有：把 `forward` 加了 `targets=None` 参数让它返回 loss，加了权重初始化（GPT-2 / nanoGPT 那一套 N(0, 0.02)）。**一行 model 类的代码都没复制**。
- **往上（L6）**：训完的 `ckpt.pt` 通过 `MODEL_PATH=...` env var 让 L6 加载。L6 内部把我们的小架构搬进 HF 的 `GPT2LMHeadModel` 壳子（保留 KV cache 等推理优化）。同一份权重，两个 engine。
