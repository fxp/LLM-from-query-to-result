...
**一句话**：把用户的一句话变成 prompt，通过 HTTP 流式问 L6，把吐回来的 token 一个个抛给上层。

源码：[`02_agent/`](https://github.com/fxp/LLM-from-query-to-result/tree/main/02_agent)

## 这一层为什么存在

L2 实现了 transformer，L6 把它包成一个能接受 prompt 的 HTTP 服务。但用户不会直接发 `POST /generate` JSON——他在浏览器里输入 "巴黎在哪"。中间需要一层做：

1. **prompt framing**：base LM（GPT-2）没有 `user` / `assistant` 概念，只能续写文本。要把"巴黎在哪"包装成 `Question: 巴黎在哪\nAnswer:` 这种形式，让模型从语料里学到的"问答"模式接续下去。
2. **streaming relay**：把 L6 的 SSE 帧解析成结构化事件，给 L7 用。
3. **失败兜底**：L6 挂了的话，给 L7 一个清晰的错误。

`agent.py` 就这点东西，~90 行。

## 为什么不是 "Agent"

最初的设计想做完整的 Plan → Act → Observe 循环（用 `write_file` / `run_shell` 真造一个 Todo 网站）。但这个 repo 的招牌是 "LLM from scratch"——L2 自己实现 GPT-2 small (124M, 2019)。GPT-2 做不了 agent：
...
```
[L8] prompt: 'Question: What is the capital of France?\nAnswer:'
...
GPT-2 124M 答得很差是正常的——这台模型 2019 年发布，没经过 RLHF/SFT。重点不是答案多好，是**这条链路里没有任何外部 API**：每个 token 都是 L2 的 GPT-2 forward pass 在 L1 的 matmul 上算出来的。
...
- **往上（L7 App）**：`run_agent(query) -> Iterator[Event]`，事件类型 `token / done / error`。
- **往下（L6 Model）**：一个 `POST /generate` SSE 请求。这个 HTTP 调用就是"让 L6 跑一次 transformer forward，把 token stream 回来"。

## 这一层的"最小"在哪里

- **没有对话历史**：每次 `run_agent` 都是一次 one-shot。要做多轮，把历史拼到 prompt 里就行（L6 会重新 prefill；要省钱就上 prefix cache，那是 L6 的活）。
...
[← L7 · App / Web UI](01-app.md) | 下一层 → [L6 · 推理服务](03-model.md)
