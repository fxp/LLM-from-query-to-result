...
> 注：当前版本不做 agentic 任务（`write_file` / `run_shell`），因为 GPT-2 124M 不具备 tool-use 能力。详见 `02_agent/README.md` "为什么不是 Agent"。

## 和其他层的接口

- **往上（用户）**：HTTP + SSE。
- **往下（L2 Agent）**：一个 Python 生成器 `agent.run(query) -> Iterator[Event]`，每个 Event 是 `{"type": "token"|"done"|"error", ...}`。
